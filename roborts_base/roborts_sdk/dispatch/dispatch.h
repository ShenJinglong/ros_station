...
public:
    ...
    SubscriptionCallback(CallbackType function): callback_function_(function) {

    }
    ~SubscriptionCallback() = default;
    
    void Dispatch(std::shared_ptr<MessageHeader> message_header, SharedMessage message) {
...
public:
    ...
    ServiceCallback(CallbackType function): callback_function_(function) {
...
public:
    SubscriptionBase(std::shared_ptr<Handle> handle, 
                    ...
                    uint8_t sender, uint8_t receiver): handle_(handle) {
...
public:
    ...
    Subscription(std::shared_ptr<Handle> handle, uint8_t cmd_set, uint8_t cmd_id,
                    uint8_t sender, uint8_t receiver, CallbackType &&function):
...
class PublisherBase {
public:
    PublisherBase(std::shared_ptr<Handle> handle, uint8_t cmd_set, uint8_t cmd_id,
                    uint8_t sender, uint8_t receiver): handle_(handle) {
        cmd_info_ = std::make_shared<CommandInfo>();
        cmd_info_->cmd_id = cmd_id;
        cmd_info_->cmd_set = cmd_set;
...
public:
    Publisher(std::shared_ptr<Handle> handle, uint8_t cmd_set, uint8_t cmd_id,
...
class ClientBase {
public:
    ClientBase(std::shared_ptr<Handle> handle, uint8_t cmd_set, uint8_t cmd_id,
                uint8_t sender, uint8_t receiver): handle_(handle) {
        cmd_info_ = std::make_shared<CommandInfo>();
        cmd_info_->cmd_id = cmd_id;
        cmd_info_->cmd_set = cmd_set;
...
public:
    ...
    Client(std::shared_ptr<Handle> handle, uint8_t cmd_set, uint8_t cmd_id,
            uint8_t sender, uint8_t receiver): 
...
class ServiceBase {
    public:
    ServiceBase(std::shared_ptr<Handle> handle, uint8_t cmd_set, uint8_t cmd_id,
                uint8_t sender, uint8_t receiver): handle_(handle) {
        cmd_info_ = std::make_shared<CommandInfo>();
        cmd_info_->cmd_id = cmd_id;
        cmd_info_->cmd_set = cmd_set;
...
public:
    ...
    Service(std::shared_ptr<Handle> handle, uint8_t cmd_set, uint8_t cmd_id,
            uint8_t sender, uint8_t receiver, CallbackType &&function):
    ...
    void HandleRequest(std::shared_ptr<MessageHeader> request_header,
    ...
    void SendResponse(std::shared_ptr<MessageHeader> request_header,
